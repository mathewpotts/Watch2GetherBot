...
async def get_w2g_channel():
    ...
    bot_msgs = list(await
                    channel.history(limit=50).filter(check_if_bot).flatten())
    ...
    if len(bot_scheduled_msgs) != 0:
        ...
        log.write(
            f"No Last Scheduled Post! Setting Scheduled Post time: {last_scheduled}",
            warn=True)

    last_embed = bot_msgs[
        0].embeds  # last !w2g command, may include user commands that won't have specific time...
...
async def background_task():
    while True:
        ...
        while t_diff.total_seconds(
        ) < 0:  # if the time difference is less than 0, set target_time to one day from preliminary target_time. Fixed a bug that caused the target time to be zero, added while loop
            target_time = datetime.combine(
                target_time.date() + timedelta(days=1), target_time.time())
            log.write(
                f't_diff less than 0 ({t_diff.total_seconds()/3600} hours). Modifying target_time.',
                warn=True)
            t_diff = target_time - now
