...
async def get_w2g_channel():
    ...
    bot_msgs = list(await channel.history(limit=50).filter(check_if_bot).flatten())
    ...
    bot_scheduled_msgs = [msg for msg in bot_msgs if '17:00:0' in str(msg.created_at) or '05:00:0' in str(msg.created_at)]
    ...
    if len(bot_scheduled_msgs) != 0:
        ...
        log.write(f"No Last Scheduled Post! Setting Scheduled Post time: {last_scheduled}",warn=True)
